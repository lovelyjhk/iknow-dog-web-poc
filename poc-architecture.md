# AI 반려견 영상 건강관리앱 POC 아키텍처

문서 기준일: 2026-05-05

## 1. POC 목표

`나아파`의 첫 POC는 종합 펫 플랫폼이 아니라, 반려견 영상을 기반으로 건강 이상 신호를 빠르게 확인하는 앱이다.
...
앱 콘셉트는 "AI 반려견 영상 건강관리앱"으로 잡되, 실제 서비스 문구와 API 출력은 법적 리스크를 줄이기 위해 "질병명 확정 판단"이 아니라 "이상 신호 감지", "위험도 안내", "수의사 상담 권장"으로 설계한다.
...
```json
{
  ...
  "disclaimer": "이 분석은 질병명의 확정 판단이 아니며 수의사의 진료를 대체하지 않습니다."
}
```
...
모바일 웹앱 POC 범위는 다음까지 확장한다.

```text
반려견 프로필
-> 매일 건강 체크
-> 보행/증상 영상 업로드 또는 URL 분석
-> 음식 안전 검색
-> 병원 상담 전 리포트
-> 분석/기록 이력
-> 크레딧/쿠폰 요금제
...
```text
Identity: 보호자, 반려견, 구독 상태
Capture: 건강 로그, 영상, 보호자 메모
Analyze: 위험도, 이상 신호, 권장 행동
Guide: 음식 안전, 병원 상담 기준, 리포트
Operate: 크레딧, 쿠폰, 이력, 고지문
Expand: 병원 제휴, 보험 청구 보조, AI Worker
```
